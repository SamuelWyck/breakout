...
std::pair<float, float> LiveTextDisplay::topleft() const {
...
std::pair<float, float> LiveTextDisplay::center() const {
...
float LiveTextDisplay::width() const {
    if (!isValid()) {
        return 0;
    }
...
float LiveTextDisplay::height() const {
    if (!isValid()) {
        return 0;
    }
...
void LiveTextDisplay::setX(float x) {
    m_useCenter = false;
    m_rect.setX(x);
};


void LiveTextDisplay::setY(float y) {
    m_useCenter = false;
    m_rect.setY(y);
};
