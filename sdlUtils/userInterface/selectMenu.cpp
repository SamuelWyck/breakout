...
MenuReturn SelectMenu::run(SDL_Renderer* renderer, int framerate, SDL_Surface* bgCanvas) {
...
int SelectMenu::runUpdate(SDL_Renderer* renderer, const SDL_FPoint& mousePos, bool mousePressed, bool mouseReleased) {
    if (m_bgImage) {
        SDL_RenderTexture(renderer, m_bgImage, nullptr, &m_bgImageRect.getSDLFRect());
    }
...
void SelectMenu::setHighestUnlockedBtnId(int highestUnlockedId) {
...
void SelectMenu::loadHighestUnlockedId() {
...
void SelectMenu::saveHighestUnlockedId() {
...
void SelectMenu::drawLockedImg(SDL_Renderer* renderer, Button* btnPtr) {
    if (btnUnlocked(btnPtr->id())) {
        return;
    }
...
bool SelectMenu::btnUnlocked(int btnId) {
    if (!m_btnLockedImg) {
        return true;
    }
...
void SelectMenu::pageButtons(bool pageUp) {
...
void SelectMenu::positionPageAndExitBtns(
    Button* exitBtn, 
    Button* pageUpBtn, 
    Button* pageDownBtn, 
    float colHeight, 
...
float SelectMenu::positionBtns(int numRows, int numCols, float centerX, float centerY, float btnGap) {
...
std::pair<float, float> SelectMenu::maxBtnSize() {
    float maxWidth {0};
    float maxHeight {0};
